...
public class DorisHttpClientFactoryTest {
    ...
    @Test
    public void apacheClientTrustsConfiguredCa() throws Exception {
        try (HttpsTestServer server = new HttpsTestServer()) {
            Assert.assertEquals(
                    200,
                    executeApache(
                            server.getUrl("localhost"),
                            HttpsTestServer.tlsOptions("/tls/ca.pem", false)));
        }
    }

    @Test
    public void sharedHttpUtilsUsesConfiguredCa() throws Exception {
        try (HttpsTestServer server = new HttpsTestServer();
                CloseableHttpClient client =
                        HttpUtils.getHttpClient(
                                HttpsTestServer.tlsConfig("/tls/ca.pem", false));
                CloseableHttpResponse response =
                        client.execute(new HttpGet(server.getUrl("localhost")))) {
            Assert.assertEquals(200, response.getStatusLine().getStatusCode());
        }
    }

    @Test
    public void urlConnectionTrustsConfiguredCa() throws Exception {
        try (HttpsTestServer server = new HttpsTestServer()) {
    ...
    @Test
    public void endpointProbeUsesTlsPolicy() throws Exception {
        try (HttpsTestServer server = new HttpsTestServer()) {
            Assert.assertTrue(
                    HttpUtil.tryHttpConnection(
    ...
    @Test
    public void wrongCaFailsHandshake() throws Exception {
        try (HttpsTestServer server = new HttpsTestServer()) {
            assertTlsFailure(
                    server.getUrl("localhost"),
                    HttpsTestServer.tlsOptions("/tls/wrong-ca.pem", false));
        }
    }

    @Test
    public void hostnameMismatchFailsByDefault() throws Exception {
        try (HttpsTestServer server = new HttpsTestServer()) {
            assertTlsFailure(
    ...
    @Test
    public void hostnameMismatchCanBeSkippedWithoutSkippingCa() throws Exception {
        try (HttpsTestServer server = new HttpsTestServer()) {
            Assert.assertEquals(
                    200,
                    executeApache(
            ...
            assertTlsFailure(
                    server.getUrl("127.0.0.1"),
                    HttpsTestServer.tlsOptions("/tls/wrong-ca.pem", true));
        }
    }

    @Test
    public void multiCertificatePemChainIsAccepted() throws Exception {
        try (HttpsTestServer server = new HttpsTestServer()) {
            Assert.assertEquals(
                    200,
                    executeApache(
    ...
    @Test
    public void rejectsHttpsToHttpRedirectBeforeSendingCredentials() throws Exception {
    ...
    private static int executeApache(String url, DorisTlsOptions options) throws IOException {
    ...
    private static void assertTlsFailure(String url, DorisTlsOptions options) throws Exception {
        try {
