 ...
 */
class TransactionHandler(committer: DorisCommitter, retries: Int, interval: Int) extends Logging with Serializable {
  ...
  private def commitTransaction(msg: CommitMessage): (Option[Any], Option[Exception]) = {
    Utils.retry(retries, Duration.ofMillis(interval), log) {
      committer.commit(msg.value.toString)
    }() match {
  ...
  def abortTransactions(messages: List[CommitMessage]): Unit = {
    ...
    val failedTxnIds = messages.map(msg =>
      Utils.retry(retries, Duration.ofMillis(interval), log) {
        committer.abort(msg.value.toString)
      }() match {
        case Success(_) => None
...
object TransactionHandler {
  def apply(committer: DorisCommitter, retries: Int, interval: Int): TransactionHandler =
    new TransactionHandler(committer, retries, interval)
}
